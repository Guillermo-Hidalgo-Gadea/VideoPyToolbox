...
- the usual pipepile to compress and concat at once
- batch rename by appending/prepending string
...
def the_usual():
    ...
    next = 'y'
    print("Ah, the usual. Comming right up.")
    ...
    while True:
        ...
        if next =='n':
            ...
            for outputfile in np.unique(concats[:,1]):
                ...
                # concatenate file
                ffmpeg_command = f"ffmpeg -progress {progress} -f concat -safe 0 -i {myfile} -an -vcodec libx265 -crf {crf} {output}"
                os.system(ffmpeg_command)
                os.remove(myfile)
